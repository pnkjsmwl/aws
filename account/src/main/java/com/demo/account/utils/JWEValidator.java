...
import com.demo.account.doc.JWTPayload;
...
public class JWEValidator{
	...
	public boolean checkPolicy(JWTPayload jwtPayload, String method, String requestURI) {
		...
		if("summary".equals(request)  && summary_Policy.contains(jwtPayload.getRole()) )
			return true;
		else if("current-balance".equals(request)  && current_balance_Policy.contains(jwtPayload.getRole()) )
			return true;
		else if("due".equals(request)  && due_Policy.contains(jwtPayload.getRole()) )
			return true;
